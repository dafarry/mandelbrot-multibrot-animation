...
blues = [(v**4, v**2.5, v) for v in np.linspace(0, 1, 128)]
sepias = [(v, v**1.5, v**3) for v in np.linspace(1, 0, 128)]
rgbcolors = np.uint8(np.array(blues + sepias) * 255)

t = 1500                                    # total frames
for f in range(t):
    #asymptotically rising power value over range 1.05 - 10000
    n = 1.05 + 2 * f / (1e-9 + t - f - 1 + (2 * t - 2) / (1e4 - 1.05))
    ...
    backgnd = np.log1p(np.fmax(0, zlog * 1.5)) * (0.15 + 0.01 * np.log(p));
    np.putmask(backgnd, mask, np.zeros_like(mask))
    tones = np.uint8(np.clip(backgnd, 0, 1) * 255)
    ...
    image.save('video/file{:04d}.png'.format(f))
